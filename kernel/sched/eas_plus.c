		...
		const struct cpumask *sg_mask, struct sg_state *sg_env)
{
	int cpu = cpumask_first(sg_mask);
        struct sched_domain *sd __attribute__((unused));
        const struct sched_group *sg __attribute__((unused));
...
{
        struct task_struct *target_tsk __attribute__((unused));
        struct rq *rq __attribute__((unused)) = cpu_rq(target_cpu);
